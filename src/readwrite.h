...
inline int write_float_le(float b, EMUFILE*is) { u64 temp = float_to_u64(b); int ret = write64le(temp,is); return ret; }
...
inline int read_float_le(float *Bufo, EMUFILE*is) { u64 temp; int ret = read64le(&temp,is); *Bufo = u64_to_float(temp); return ret; }
